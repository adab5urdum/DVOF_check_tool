    ...
    for lyr in tds_list:
        ...
        with ap.da.SearchCursor(working_layer, tds_fields, where_clause) as cursor:
            for row in cursor:
                ...
                if featuretyp == 0:
                    check_row = ['', '', '{} does not have FEATURETYP assigned.'.format(row[1]), row[-1]]
...
def populate_edits(edits_dict, dvof_fc, check_db):
    add_list = []
    tds_fields = ["OID@", "fcsubtype", "hgt", "SHAPE@"]
    otherremar = 'C'
    voidentifi = ''
...
def populate_deletes(deletes_list, source_dvof, delivery_dvof):
    ...
    field_names = [f.name for f in ap.ListFields(source_dvof)]
    if 'FID' in field_names:
        oid_field = 'FID'
    elif 'ObjectID' in field_names:
        oid_field = 'ObjectID'
    elif 'OBJECTID' in field_names:
        oid_field = 'OBJECTID'
...
def get_featuretyp_code(tds_subtype):
    fcsubtype_featuretyp_dict = {
        ...
        100025: 182, 154703: 324, 100131: 820,    # Check storage structures open/closed
        100139: 801, 100122: 501, 100337: 926, 100142: 824, 100045: 437,    # Change 437 to 438 if it does not have country code
...
def main(*argv):
    ...
    populate_edits(changes_tracker, out_dvof, out_checks)
    ...
    populate_adds(working_tds_list, ignore_tracker, out_dvof, out_checks)
